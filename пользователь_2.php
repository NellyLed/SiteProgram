...
if(mb_strlen($login) < 4 || mb_strlen($login) > 15){
	echo "<div id='ram'><h3>Недопустимая длина логина.<br><a href='польз.html'>Назад к заполнению</a></h3></div><br>";
	exit();
}
 ...
 else if(mb_strlen($name) < 4 || mb_strlen($name) > 15){
	echo "<div id='ram'><h3>Недопустимая длина имени.<br><a href='польз.html'>Назад к заполнению</a></h3></div><br>";
	exit();
}
...
if(isset($_POST['button2']))
{
	...
	//если уже существует
	if(!empty($userlogin)){
	echo "<div id='ram'><h3>Данный логин уже используется!<br>Вы можете попробовать снова, перейдя по <a style='text-decoration: underline;color:#ffbb29;' href='польз.html'>ссылке</a></h3></div>";
	exit();} 
	else{
	//если логин новый
	$sql = mysqli_query($link,"INSERT INTO `client` (`name`,`login`, `pass`) VALUES('{$_POST['f']}','{$_POST['login']}', '{$_POST['password']}')");
	if ($sql) {
		echo "<div id='ram'><h3>Ваше имя,логин и пароль записаны!<br>В личном кабинете вы сможете дополнить свои данные (телефон, почту, адрес) по <a style='text-decoration: underline;color:#ffbb29;' href='профиль.php'>ссылке</h3></div>";
		} else {
